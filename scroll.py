...
# Settings
# Can use wildcards (Must be in nick!user@host format)
admins = [
...
def is_admin(ident):
    return re.compile(',',join(admins).replace('*','.*')).search(ident)
                                ...
                                if msg == '.ascii dirs':
                                    ...
                                    if url.startswith('https://') or url.startswith('http://'):
                                        try:
                                            ...
                                            ascii   = img2irc.convert(content, 512 - len(f":{identity.nickname}!{identity.username}@{self.host} PRIVMSG {chan} :\r\n"), int(self.settings['png_width']), self.settings['png_palette'], self.settings['png_brightness'], self.settings['png_contrast'], self.settings['png_effect'], int(self.settings["png_quantize_colors"]))
                                        except Exception as ex:
                                            traceback.print_exc()
                                    ...
                                    if len(args) == 2:
                                        for item in self.settings:
                                            await self.sendmsg(chan, color(item.ljust(20), yellow) + color(str(self.settings[item]), grey))
